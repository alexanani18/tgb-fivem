...
export interface InactivityAdminWorkflow {
  id: number;
...
interface InactivityRequestAdminProps {
  workflow: InactivityAdminWorkflow;
...
export default function InactivityRequestAdmin({
  workflow,
  onWorkflowUpdated,
}: InactivityRequestAdminProps) {
  const router = useRouter();

  const [inactivity, setInactivity] = useState<InactivityRequest | null>(null);
  ...
  const loadInactivity = useCallback(
    async (showRefreshLoader = false) => {
      ...
      try {
        const response = await fetch(`${API_URL}/inactivities/${workflow.id}`, {
          method: "GET",
          credentials: "include",
          cache: "no-store",
        });

        const data = (await response.json()) as InactivityResponse;

        if (!response.ok || !data.success || !data.data) {
          throw new Error(
            data.message ??
              "Detaliile cererii de inactivitate nu au putut fi încărcate.",
          );
        }

        setInactivity(data.data);
      } catch (error) {
        console.error("Failed to load admin inactivity details:", error);
    ...
    try {
      ...
      if (modalType === "approve") {
        response = await fetch(
          `${API_URL}/inactivities/${workflow.id}/approve`,
          {
            method: "POST",
            credentials: "include",
          },
        ...
        response = await fetch(
          `${API_URL}/inactivities/${workflow.id}/reject`,
          {
            method: "POST",
            credentials: "include",
            headers: {
        ...
        response = await fetch(`${API_URL}/inactivities/${workflow.id}`, {
          method: "DELETE",
          credentials: "include",
        });
      }
      ...
      setSuccessMessage(
        data.message ??
          (modalType === "approve"
            ...
            : "Cererea de inactivitate a fost respinsă."),
      );
      ...
      await loadInactivity(true);
    } catch (error) {
      console.error("Failed to process inactivity action:", error);
    ...
    return (
      <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-8 text-center">
        ...
        <h2 className="mt-4 text-lg font-semibold text-white">
          Detaliile inactivității nu sunt disponibile
        </h2>
    ...
    <>
      ...
      <div className="space-y-6">
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
          <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 md:p-6">
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                type="button"
                onClick={() => void loadInactivity(true)}
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              ...
              <InfoCard
                ...
                label="Activitate"
                value={inactivity.activity}
              />

              <InfoCard
                icon={<CalendarDays className="h-4 w-4" />}
                label="Data activității"
                value={formatDate(inactivity.activityDate)}
              />
              ...
              <div className="mt-5 space-y-3">
                <div className="rounded-xl border border-amber-500/20 bg-amber-500/[0.06] p-4">
                  ...
                  <p className="mt-1 text-xs leading-5 text-amber-200/70">
                    Verifică activitatea, data și motivul înainte de aprobare.
                  </p>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-green-200">
                      Inactivitate aprobată
                    </p>

                    <p className="mt-1 text-xs leading-5 text-green-200/70">
                      Observația a fost adăugată automat angajatului.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <section className="rounded-2xl border border-white/10 bg-white/[0.04] p-5 md:p-6">
          ...
          <h2 className="mt-2 text-lg font-semibold text-white">
            Inactivitatea solicitată
          </h2>

          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <DetailCard label="Activitate" value={inactivity.activity} />
...
function ActionModal({
  ...
  const title = isApprove
    ? "Aprobă inactivitatea"
    : isReject
      ? "Respinge inactivitatea"
      : "Șterge cererea";
      ...
      <div className="w-full max-w-lg overflow-hidden rounded-2xl border border-white/10 bg-[#101010] shadow-2xl">
        ...
        <div className="p-5">
          <div className="rounded-xl border border-white/10 bg-black/25 p-4 text-sm text-zinc-300">
            <p>
              Activitate:{" "}
              <strong className="text-white">{inactivity.activity}</strong>
            </p>
          ...
          {isApprove && (
            <p className="mt-4 text-sm leading-6 text-zinc-300">
              Confirmi aprobarea acestei cereri de inactivitate? În observațiile
              angajatului va fi adăugat imediat:
              <strong className="mt-2 block text-white">
                INACTIVITATE PENTRU {inactivity.activity.toUpperCase()} -{" "}
                {formatDate(inactivity.activityDate)}
              </strong>
            </p>
          )}
          ...
          {isDelete && (
            <>
              ...
              {workflow.statusCode === "APPROVED" && (
                <p className="mt-4 text-sm leading-6 text-zinc-300">
                  Cererea este aprobată, deci observația asociată acestei
                  inactivități va fi eliminată automat din fișa angajatului.
                </p>
              )}
            </>
          )}
